...
private:
    ...
    {
        int32_t value = 0;
        SIMDString<64> prefName;
        SIMDString<64> iniSec;
        SIMDString<64> iniName;
        SIMDString<64> strEnum;
        ...
        auto ReadFromIni(auto& iniReader) {
          return iniReader.ReadInteger(std::string_view(iniSec),
                                       std::string_view(iniName), iniDefValInt);
        }
        auto ReadFromIni()
        {
            CIniReader iniReader(cfgPath);
            return ReadFromIni(iniReader);
        }
        void WriteToIni(auto& iniWriter) {
          iniWriter.WriteInteger(std::string_view(iniSec),
                                 std::string_view(iniName), value, true);
        }
        void WriteToIni()
        {
...
private:
    ...
    std::optional<SIMDString<64>> GetPrefNameByID(auto prefID)
    {
    ...
    std::optional<int32_t> GetPrefIDByName(auto prefName)
    {
        auto it = std::find_if(std::begin(aMenuPrefs), std::end(aMenuPrefs), [&prefName](auto &it)
                               { return SIMDString<64>(it.name) == prefName; });
        if (it != std::end(aMenuPrefs))
            return it->prefID;
        return std::nullopt;
    }
...
public:
    ...
    auto Set(int32_t prefID, int32_t value)
    {
        ...
        else
        {
            ...
            if (slidersList.contains(prefID))
            {
                ...
                if (id)
                    mFusionPrefs[*id].SetValue(value);
            }
        }
    }
    int32_t Get(SIMDString<64> name)
    {
    ...
    auto Set(SIMDString<64> name, int32_t value)
    {
        auto prefID = GetPrefIDByName(name);
        if (prefID)
            return Set(*prefID, value);
    }
    auto isSame(int32_t id, SIMDString<64> name)
    {
        auto prefID = GetPrefIDByName(name);
        if (prefID && *prefID == id)
            return true;
    ...
    std::optional<std::reference_wrapper<int32_t>> GetRef(SIMDString<64> name)
    {
        auto prefID = GetPrefIDByName(name);
        if (prefID)
        {
    ...
    void SetCallback(SIMDString<64> name, std::function<void(int32_t)> &&cb)
    {
        auto prefID = GetPrefIDByName(name);
        if (prefID)
            mFusionPrefs[*prefID].callback = cb;
    }
    ...
    auto operator()(SIMDString<64> name) { return Get(name); }
