    ...
    float* mPedAOStrength)
{
    ...
    if (VolumetricFog->get())
        *mFarClip = fVolFogFarClip;

    static auto DepthOfField = FusionFixSettings.GetRef("PREF_TCYC_DOF");
    switch (DepthOfField->get())
    {
    ...
    static auto ScreenFilter = FusionFixSettings.GetRef("PREF_TIMECYC");
    switch (ScreenFilter->get())
    {
...
public:
    Timecyc()
    {
        FusionFix::onInitEventAsync() += []()
        {
            ...
            auto pattern = find_pattern("E8 ? ? ? ? 0F B6 8C 24 ? ? ? ? 0F B6 84 24", "E8 ? ? ? ? 0F B6 84 24 ? ? ? ? 0F B6 8C 24");
            injector::MakeCALL(pattern.get_first(0), timecyc_scanf, true);

            // Z-fighting fix helpers
            {
                pattern = find_pattern("E8 ? ? ? ? 6A ? E8 ? ? ? ? 8B 0D ? ? ? ? 83 C4 ? 89 04 8D ? ? ? ? 8B 4C 24 ? 89 08 A1 ? ? ? ? F3 0F 10 44 24", 
                                            "E8 ? ? ? ? 6A ? E8 ? ? ? ? 8B 0D ? ? ? ? 8B 54 24 ? 89 04 8D ? ? ? ? 89 10 A1 ? ? ? ? 8B 0C 85 ? ? ? ? F3 0F 10 44 24");
