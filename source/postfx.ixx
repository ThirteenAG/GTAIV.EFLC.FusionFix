...
public:
    ...
    bool loadShaders(LPDIRECT3DDEVICE9 pDevice, HMODULE hm) {
        ...
        if(!deferred_lighting_PS1 && D3DXAssembleShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_deferred_lighting_PS1), NULL, NULL, 0, &bf1, &bf2) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &deferred_lighting_PS1) != S_OK || !deferred_lighting_PS1) SAFE_RELEASE(deferred_lighting_PS1); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); }
        if(!deferred_lighting_PS2 && D3DXAssembleShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_deferred_lighting_PS2), NULL, NULL, 0, &bf1, &bf2) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &deferred_lighting_PS2) != S_OK || !deferred_lighting_PS2) SAFE_RELEASE(deferred_lighting_PS2); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); }
        if(!SSAO_gen_ps && D3DXAssembleShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_SSAO_gen_ps), NULL, NULL, 0, &bf1, &bf2) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &SSAO_gen_ps) != S_OK || !SSAO_gen_ps) SAFE_RELEASE(SSAO_gen_ps); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); }
        if(!SSAO_blend_ps && D3DXAssembleShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_SSAO_blend_ps), NULL, NULL, 0, &bf1, &bf2) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &SSAO_blend_ps) != S_OK || !SSAO_blend_ps) SAFE_RELEASE(SSAO_blend_ps); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); }
        ...
        if(!DeferredShadowBlurH_ps && D3DXCompileShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_DeferredShadowBlur), NULL, NULL, "BlurHorizontal", "ps_3_0", 0, &bf1, &bf2, &ppConstantTable) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &DeferredShadowBlurH_ps) != S_OK || !DeferredShadowBlurH_ps) SAFE_RELEASE(DeferredShadowBlurH_ps); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); SAFE_RELEASE(ppConstantTable); }
        if(!DeferredShadowBlurV_ps && D3DXCompileShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_DeferredShadowBlur), NULL, NULL, "BlurVertical", "ps_3_0", 0, &bf1, &bf2, &ppConstantTable) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &DeferredShadowBlurV_ps) != S_OK || !DeferredShadowBlurV_ps) SAFE_RELEASE(DeferredShadowBlurV_ps); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); SAFE_RELEASE(ppConstantTable); }
        if(!DeferredShadowBlurCircle_ps && D3DXCompileShaderFromResourceW(hm, MAKEINTRESOURCEW(IDR_DeferredShadowBlur), NULL, NULL, "BlurCircular", "ps_3_0", 0, &bf1, &bf2, &ppConstantTable) == S_OK) { if(CreatePixelShaderOriginal.unsafe_stdcall<HRESULT>(pDevice, (DWORD*) bf1->GetBufferPointer(), &DeferredShadowBlurCircle_ps) != S_OK || !DeferredShadowBlurCircle_ps) SAFE_RELEASE(DeferredShadowBlurCircle_ps); SAFE_RELEASE(bf1); SAFE_RELEASE(bf2); SAFE_RELEASE(ppConstantTable); }
...
#define PostfxTextureCount 15
    ...
    void Readini() {
        ...
        EnablePostfx = iniReader.ReadInteger("SRF", "EnablePostfx", 1) != 0;
    }
};
        ...
        FusionFix::onInitEvent() += [] () {
            ...
            if(PostFxResources.EnablePostfx) {
                ...
                FusionFix::D3D9::onBeforeDrawPrimitive() += [] (LPDIRECT3DDEVICE9& pDevice, D3DPRIMITIVETYPE& PrimitiveType, UINT& StartVertex, UINT& PrimitiveCount) {
                    ...
                    // any postfx shader ID, apply new post processing steps here
                    if(id == 815 || id == 817 || id == 819 || id == 821 || id == 827 || id == 829 || id == 831 && PostFxResources.shadersLoaded) {
                        hr = PostFx3(pDevice, PrimitiveType, StartVertex, PrimitiveCount, pShader, vShader);
                        FusionFix::D3D9::setInsteadDrawPrimitive(true);
                    }

                    // atmoscat clouds, saves the result of 'sun * (1-clouds.a)' in the diffuse texture
                    else if((id == 65 || id == 66) && PostFxResources.DiffuseTex != nullptr && PostFxResources.shadersLoaded) {
                    ...
                    // render the shadow atlas to a depth texture to enable hardware bilinear filtering during sampling
                    else if(PostFxResources.useHardwareBilinearSampling == true && id == 1 && PostFxResources.NewShadowAtlas && PostFxResources.shadersLoaded) // cascade convert
                    {
                    ...
                    // screen space shadow blur // no more break lights
                    else if(PostFxResources.useScreenSpaceShadowsBlur > 0 && id == 6 || id == 7 && PostFxResources.shadersLoaded) // deferred_lightingPS 1 e 2 
                    {
                        ...
                        if(PostFxResources.pShadowBlurTex1 && PostFxResources.pShadowBlurTex2) {
                            ...
                            if(pShadowBlurSurf1 && pShadowBlurSurf2 && OldRenderTarget) {
                                ...
                                pDevice->GetSamplerState(11, D3DSAMP_MAGFILTER, &old_SAMP_MAGFILTER);
                                pDevice->SetSamplerState(11, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);

                                if(PostFxResources.useHardwareBilinearSampling == true && PostFxResources.NewShadowAtlas) {
                                ...
                                if(PostFxResources.useScreenSpaceShadowsBlur == 1) {
                                    pDevice->SetRenderTarget(0, pShadowBlurSurf2);
                                    pDevice->SetTexture(11, PostFxResources.pShadowBlurTex1);
                                    pDevice->SetPixelShader(PostFxResources.DeferredShadowBlurH_ps);
                                    DrawPrimitiveOriginal.unsafe_stdcall<HRESULT>(pDevice, PrimitiveType, StartVertex, PrimitiveCount);
                                }
                                if(PostFxResources.useScreenSpaceShadowsBlur == 2) {
                                    pDevice->SetRenderTarget(0, pShadowBlurSurf2);
                                    pDevice->SetTexture(11, PostFxResources.pShadowBlurTex1);
                                    pDevice->SetPixelShader(PostFxResources.DeferredShadowBlurV_ps);
                                    DrawPrimitiveOriginal.unsafe_stdcall<HRESULT>(pDevice, PrimitiveType, StartVertex, PrimitiveCount);
                                }
                                if(PostFxResources.useScreenSpaceShadowsBlur == 3) {
                                    pDevice->SetPixelShader(PostFxResources.DeferredShadowBlurH_ps);
                                    pDevice->SetTexture(11, PostFxResources.pShadowBlurTex1);
                                    pDevice->SetRenderTarget(0, pShadowBlurSurf2);
                                    DrawPrimitiveOriginal.unsafe_stdcall<HRESULT>(pDevice, PrimitiveType, StartVertex, PrimitiveCount);
                                    ...
                                    pDevice->SetPixelShader(PostFxResources.DeferredShadowBlurV_ps);
                                    pDevice->SetTexture(11, PostFxResources.pShadowBlurTex2);
                                    pDevice->SetRenderTarget(0, pShadowBlurSurf1);
                                    DrawPrimitiveOriginal.unsafe_stdcall<HRESULT>(pDevice, PrimitiveType, StartVertex, PrimitiveCount);
                                }
                                if(PostFxResources.useScreenSpaceShadowsBlur == 4) {
                                    pDevice->SetRenderTarget(0, pShadowBlurSurf2);
                                    pDevice->SetTexture(11, PostFxResources.pShadowBlurTex1);
                                ...
                                if(id == 6)
                                    pDevice->SetPixelShader(PostFxResources.deferred_lighting_PS1);
                                else
                                    pDevice->SetPixelShader(PostFxResources.deferred_lighting_PS2);
                                DrawPrimitiveOriginal.unsafe_stdcall<HRESULT>(pDevice, PrimitiveType, StartVertex, PrimitiveCount);
                                pDevice->SetTexture(11, 0);
                                pDevice->SetPixelShader(pShader);
                                pDevice->SetSamplerState(11, D3DSAMP_MAGFILTER, old_SAMP_MAGFILTER);
    ...
    static HRESULT PostFx3(LPDIRECT3DDEVICE9 pDevice, D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex,
        ...
        // main postfx passes
        {
            ...
            if(PostFxResources.pHDRTex2 && PostFxResources.pHDRTex && PostFxResources.aoTex) {
                ...
                if(backBuffer && PostFxResources.renderTargetSurf && PostFxResources.surfaceRead) {
                    if(PostFxResources.UseSSAO && PostFxResources.SSAO_gen_ps && PostFxResources.SSAO_blend_ps && pShadowBlurSurf1 && pShadowBlurSurf2) {

                        pDevice->SetPixelShader(PostFxResources.SSAO_gen_ps);
                        vec4[0] = getWindowWidth();
                        vec4[1] = getWindowHeight();
                        vec4[2] = 1.f / getWindowWidth();
                        vec4[3] = 1.f / getWindowHeight();
