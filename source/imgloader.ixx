            ...
            struct ImgListHook
            {
                void operator()(injector::reg_pack &regs)
                {
                    *(uint32_t *)(regs.esp + 0x44) = regs.eax;
                    ...
                    {
                        if (SIMDString<64>((const char *)regs.eax).contains(":"))
                            return;
                    }
                        ...
                        for (const auto &file : std::filesystem::recursive_directory_iterator(updatePath, perms))
                        {
                            ...
                            if (!std::filesystem::is_directory(file) && iequals(filePath.extension().native(), L".img"))
                            {
                                ...
                                if (pos != imgPath.npos)
                                {
                                    ...
                                    if (std::any_of(std::begin(episodicPaths), std::end(episodicPaths), [&](auto &it)
                                                    { return contains_subfolder(relativePath, it); }))
                                    {
