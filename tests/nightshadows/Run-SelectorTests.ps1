...
try {
    $compilerCommand = '"' + $setup + '" >nul && cl /nologo /std:c++17 /W4 /WX /EHsc /O2 selector_tests.cpp /Fe:selector_tests-' + $Architecture + '.exe /Fo:selector_tests-' + $Architecture + '.obj && selector_tests-' + $Architecture + '.exe'
    & $env:ComSpec /d /c $compilerCommand
    if ($LASTEXITCODE -ne 0) { throw ('Selector tests failed with exit code ' + $LASTEXITCODE) }
}
